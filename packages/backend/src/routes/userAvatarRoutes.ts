...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error.code === 'P2002') { // Unique constraint failed (should be caught by pre-check, but as a fallback)
...
router.delete('/:userAvatarId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (userAvatarEntry.userId !== userId) {
        ...
        return res.status(403).json({ error: 'Forbidden: This avatar ownership record does not belong to the specified user.' });
    }
