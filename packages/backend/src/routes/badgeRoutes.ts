...
router.post('/', async (req: Request, res: Response) => {
  ...
  const { languageLevel, badgeUrl } = req.body;
  ...
  if (!languageLevel || !badgeUrl) {
    return res.status(400).json({ error: 'languageLevel and badgeUrl are required.' });
  }
  ...
  try {
    ...
    if (error.code === 'P2002') { // Unique constraint failed
        return res.status(409).json({ error: `User already has a badge for level ${languageLevel} (database constraint).`})
    }
...
router.get('/:level', (req: Request, res: Response) => {
  ...
  if (userId) {
    ...
    // For now, let's proceed as if this is only for generic /api/badges/:level
     return res.status(404).json({ error: "Endpoint not found for user-specific level badge info. Try /api/users/:userId/badges?languageLevel=X" });
  }
