...
router.post('/', async (req: Request, res: Response) => {
  ...
  const { languageLevel, badgeUrl, tokenId } = req.body
  ...
  if (!languageLevel || !badgeUrl || !tokenId) {
    return res.status(400).json({ error: 'languageLevel and badgeUrl and tokenId are required.' })
  }
