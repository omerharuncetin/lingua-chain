...
router.post('/', async (req: Request, res: Response) => {
  ...
  const { languageLevel, certificateUrl, tokenId } = req.body

  if (!userId) {
    return res.status(400).json({ error: 'User ID is required in the path.' })
  }
  if (!languageLevel || !certificateUrl || !tokenId) {
    return res.status(400).json({ error: 'languageLevel and certificateUrl and tokenId are required.' })
  }
  ...
  try {
    ...
    if (error.code === 'P2002') {
      return res
        .status(409)
        .json({ error: `User already has a certificate for level ${languageLevel} (database constraint).` })
    }
...
router.get('/:level', (req: Request, res: Response) => {
  ...
  if (userId) {
    ...
    return res.status(404).json({
      error:
        'Endpoint not found for user-specific level certificate info. Try /api/users/:userId/certificates?languageLevel=X',
    })
  }
