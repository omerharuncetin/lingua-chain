...
router.post('/:userId/complete-lesson', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { id: userId },
      include: {
        equippedAvatar: { // Include the UserAvatar record
          include: {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({
      ...
      include: {
        progress: true,
        leaderboard: true,
        avatars: { // User's owned UserAvatar records
          include: {
        ...
        certificates: true,
        badges: true,
        equippedAvatar: { // The specific UserAvatar record that is equipped
          include: {
            avatar: true, // Include the base Avatar details for the equipped UserAvatar
          },
        },
        dailyLessons: { // Fetch only today's lesson record
          where: {
            date: today,
          },
...
router.post('/:userId/equip-avatar', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error.code === 'P2002' && error.meta?.target?.includes('equippedAvatarId')) {
      ...
      return res.status(409).json({ error: 'This specific UserAvatar instance is already equipped by a user (should not happen if UserAvatar instances are not shared). Or, trying to set an equippedAvatarId that another user record is already using for their equipped avatar.' });
    }
    if (error.code === 'P2025') {
