...
async function handleTransferEvent(log: Log, contract: MonitoredContract) {
  const topic = decodeEventLog({
    abi: genericErc721TransferAbi,
    topics: log.topics,
    data: log.data,
  })

  if (topic.eventName !== 'Transfer') {
    return
  }

  const { from, to, tokenId } = topic.args

  // We are only interested in mints for SBTs
  if (from !== ZERO_ADDRESS) {
    console.log(
      `[SBT Listener - ${contract.contractName}] Ignoring non-mint transfer for token ${tokenId} from ${from} to ${to}.`
    )
    return
  }
  ...
  try {
    ...
    if (!user) {
      console.log(
        `[SBT Listener - ${contract.contractName}] User with wallet address ${to} not found. Skipping badge/certificate creation.`
      )
      return
    }

    const tokenIdStr = tokenId.toString()
    const metadataUrl = `${process.env.API_BASE_URL}/api/nft/${contract.sbtType.toLowerCase()}s/${contract.languageLevel.toLowerCase()}/${tokenIdStr}`
    ...
    if (contract.sbtType === 'Badge') {
      ...
      console.log(
        `[SBT Listener - ${contract.contractName}] Badge for level ${contract.languageLevel.toUpperCase()} (Token: ${tokenIdStr}) processed for user ${user.id} (${to}). URL: ${metadataUrl}`
      )
      ...
      console.log(
        `[SBT Listener - ${contract.contractName}] Certificate for level ${contract.languageLevel.toUpperCase()} (Token: ${tokenIdStr}) processed for user ${user.id} (${to}). URL: ${metadataUrl}`
      )
    }
  } catch (error: any) {
    if (error.code === 'P2002') {
      ...
      console.warn(
        `[SBT Listener - ${contract.contractName}] Failed to record ${contract.sbtType} for user ${to}, level ${contract.languageLevel} (Token ${tokenId.toString()}) due to unique constraint. It might already exist if upsert logic isn't perfect for the case or if a race condition occurred. Error: ${error.message}`
      )
    } else {
      console.error(
        `[SBT Listener - ${contract.contractName}] Error processing Transfer event for token ${tokenId}:`,
        error
      )
    }
  }
...
export function initializeSbtListeners() {
  ...
  monitoredContracts.forEach((contract) => {
    console.log(
      `[SBT Listener Service] Setting up listener for ${contract.contractName} (${contract.contractAddress}) - Type: ${contract.sbtType}, Level: ${contract.languageLevel}`
    )
    ...
    try {
      ...
      console.log(
        `[SBT Listener Service] Successfully started listener for Transfer events on ${contract.contractName}.`
      )
    } catch (error) {
